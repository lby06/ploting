...
big_M = 1e6
gas_carbon = 0.2165 # 燃气碳排放系数，单位吨/MWh
...
def read_data(city_code):
    ...
    file_name2 = f"/Users/liuboyuan/Desktop/ploting/data/carbon_factor/normalized_carbon_factors.csv"
    # 读取数据
...
def optimal_plan(city_code, carbon_price, pv_space):
    # 记录开始的时间
    ...
    elec_load, heat_load, cold_load, elec_price, gas_price, pv_I, elec_carbon = read_data(city_code)
    ...
    #不使用季节性储能 约束为0
    def c_seasonal_heat_storage_used(model):
        m = model
        return m.seasonal_heat_storage_used == 0
    m.c_seasonal_heat_storage_used = Constraint(rule=c_seasonal_heat_storage_used)
...
#----------------------------------------
    ...
    #设定季节性储能投资为0
    def c_seasonal_heat_storage_invest(model):
        m = model
        seasonal_heat_storage_index = next(i for i, device in enumerate(m.storage_device_list) if device.label == "Seasonal_Heat_Storage")
        return m.storage_invest[seasonal_heat_storage_index] == 0
    m.c_seasonal_heat_storage_invest = Constraint(rule=c_seasonal_heat_storage_invest)
    ...
    # 定义能源平衡约束
    def seasonal_heat_storage_0(model):
        m = model
        seasonal_heat_storage_index = next(i for i, device in enumerate(m.storage_device_list) if device.label == "Seasonal_Heat_Storage")
        def rule(m, t):
            return m.storage_charge_power[seasonal_heat_storage_index, t] == 0
        return rule

    m.seasonal_heat_storage_0 = Constraint(m.t_8760, rule=seasonal_heat_storage_0(m))
    def seasonal_heat_storage_1(model):
        m = model
        seasonal_heat_storage_index = next(i for i, device in enumerate(m.storage_device_list) if device.label == "Seasonal_Heat_Storage")
        def rule(m, t):
            return m.storage_discharge_power[seasonal_heat_storage_index, t] == 0
        return rule

    m.seasonal_heat_storage_1 = Constraint(m.t_8760, rule=seasonal_heat_storage_1(m))
...
if __name__ == '__main__':
    carbon_price = 0
    pv_space = 3.8
    # typical_week_selection()
    for city_code in range(1, 5):
        #city_code = 1
        ...
        for device_id in range(len(m.conversion_device_list)):
            the_device = m.conversion_device_list[device_id]
            total_cost += the_device.cost * m.convert_invest[device_id]()

        # 储能设备
        for device_id in range(len(m.storage_device_list)):
            the_device = m.storage_device_list[device_id]
            if the_device.label == "Seasonal_Heat_Storage":
                total_cost += old_equal(116.32, 30) * m.seasonal_heat_storage_used + the_device.cost * m.storage_invest[device_id]()
            else:
                total_cost += the_device.cost * m.storage_invest[device_id]()
        #----------------------------------------
        # 总购电成本计算
        total_buy_cost = 0
        for t in m.t_8760:
            # 统一为万元单位
            # 购电成本
            total_buy_cost += m.elec_price[t] * m.buy_energy["elec", t]() / 10000
            total_buy_cost += m.gas_price[t] * m.buy_energy["gas", t]() / 10000
        print('总购电成本:{}'.format(total_buy_cost))
        #----------------------------------------
        # 总弃负荷成本计算
        total_shed_cost = 0
        for t in m.t_8760:
            total_shed_cost += m.shed_energy["elec", t]() * 1e3
            total_shed_cost += m.shed_energy["heat", t]() * 1e3
            total_shed_cost += m.shed_energy["cold", t]() * 1e3
        print('总弃负荷成本:{}'.format(total_shed_cost))
        #----------------------------------------
        # 各个设备容量规划结果
        print('转换设备容量:')
        for device in m.set_converter:
            print('设备{}，数量:{}，容量:{}'.format(m.conversion_device_list[device].label, m.convert_invest[device](), \
                m.convert_invest[device]()))
        print('储能设备容量:')
        for device in m.set_storage:
            print('设备{}，数量:{}，功率容量:{}'.format(m.storage_device_list[device].label, m.storage_invest[device](), \
                m.storage_invest[device]()/m.storage_device_list[device].t_duration))
        #---------------------------------
        # 光伏发电和太阳能集热器的面积
        print('光伏发电面积: {}'.format(m.solar_Area[0]()))
        print('太阳能集热器面积: {}'.format(m.solar_Area[1]()))

        # 计算全年冷负荷的总流入量，并分别标注每一部分冷注入的来源
        total_cold_inflow = 0
        conversion_cold_inflow = 0
        storage_cold_inflow = 0
        storage_cold_outflow = 0

        for device in m.conversion_device_list:
            if "cold" in device.output_kind:
                output_key = device.label + "_cold"
                device_cold_inflow = sum(m.convert_power[output_key, t]() for t in m.t_8760)
                conversion_cold_inflow += device_cold_inflow
                print(f'转换设备 {device.label} 的冷注入量: {device_cold_inflow}')

        for device_id, device in enumerate(m.storage_device_list):
            if device.output_kind == "cold":
                device_cold_inflow = sum(m.storage_discharge_power[device_id, t]() for t in m.t_8760)
                storage_cold_inflow += device_cold_inflow
                print(f'储能设备 {device.label} 的冷注入量: {device_cold_inflow}')
        for device_id, device in enumerate(m.storage_device_list):
            if device.output_kind == "cold":
                device_cold_outflow = sum(m.storage_charge_power[device_id, t]() for t in m.t_8760)
                storage_cold_outflow += device_cold_outflow
                print(f'储能设备 {device.label} 的冷注入量: {device_cold_inflow}')

        total_cold_inflow = conversion_cold_inflow + storage_cold_inflow
        print('全年冷负荷的总流入量: {}'.format(total_cold_inflow))

        # 计算全年冷负荷的总输出量
        total_cold_outflow = sum(m.cold_load[t] for t in m.t_8760) + storage_cold_outflow
        print('全年冷负荷的总输出量: {}'.format(total_cold_outflow))

        # 计算全年电负荷的总流入量，并分别标注每一部分电注入的来源
        total_elec_inflow = 0
        conversion_elec_inflow = 0
        storage_elec_inflow = 0
        storage_elec_outflow = 0
        buy_elec_inflow = sum(m.buy_energy["elec", t]() for t in m.t_8760)
        pv_elec_inflow = sum(m.solar_energy["elec", t]() for t in m.t_8760)
        conversion_elec_outflow = 0

        for device in m.conversion_device_list:
            if "elec" in device.output_kind:
                output_key = device.label + "_elec"
                device_elec_inflow = sum(m.convert_power[output_key, t]() for t in m.t_8760)
                conversion_elec_inflow += device_elec_inflow
                print(f'转换设备 {device.label} 的电注入量: {device_elec_inflow}')
            if device.input_kind == "elec":
                input_key = device.label + "_elec"
                device_elec_outflow = sum(m.convert_power[input_key, t]() for t in m.t_8760)
                conversion_elec_outflow += device_elec_outflow
                print(f'转换设备 {device.label} 的电耗量: {device_elec_outflow}')

        for device_id, device in enumerate(m.storage_device_list):
            if device.output_kind == "elec":
                device_elec_inflow = sum(m.storage_discharge_power[device_id, t]() for t in m.t_8760)
                storage_elec_inflow += device_elec_inflow
                print(f'储能设备 {device.label} 的电注入量: {device_elec_inflow}')
        for device_id, device in enumerate(m.storage_device_list):
            if device.input_kind == "elec":
                device_elec_outflow = sum(m.storage_charge_power[device_id, t]() for t in m.t_8760)
                storage_elec_outflow += device_elec_outflow
                print(f'储能设备 {device.label} 的电输出量: {device_elec_outflow}')

        total_elec_inflow = conversion_elec_inflow + storage_elec_inflow + buy_elec_inflow + pv_elec_inflow
        print('全年电负荷的总流入量: {}'.format(total_elec_inflow))

        # 计算全年电负荷的总输出量
        total_elec_outflow = sum(m.elec_load[t] for t in m.t_8760) + storage_elec_outflow + conversion_elec_outflow
        print('全年电负荷的总输出量: {}'.format(total_elec_outflow))
